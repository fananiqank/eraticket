...
$con=new Database();
	...
	// //die;
        ...
        $jmldetail=$con->select("trdata a left join mtdept b on a.departemen_data=b.id_dept
        	...
        	"a.*,b.nama_dept,e.nama_pegawai,c.note_agent,c.priority_wo,f.note_user,c.id_agent as agentwo,c.id_wo",
        	"a.id_data='$iddata'");   
        $no=1;
     ...
     // echo "select a.*,b.nama_dept from trdata a left join mtdept b on a.departemen_data=b.id_dept where a.no_ticket='$idgen'";            
   		$selfb = $con->select("trfeedback","*","id_wo = '$d[id_wo]'","id_feedback DESC");
		foreach ($selfb as $fb) {}
        ...
        $asign=$con->select("(
			...
			left join mauser_login f on e.id_pegawai = f.id_pegawai
     		where f.ID_ROLE = '1' and f.STATUS = '1'
     		union 
     		select d.id_agent as agent,e.email as email,e.nama_pegawai as nama FROM
			mtagent d left join mtpegawai e on d.id_pegawai = e.id_pegawai
			left join mauser_login f on e.id_pegawai = f.id_pegawai
     		where id_agent = $d[agentwo]) as b","agent,email,nama"); 
        ...
        foreach($asign as $is){
			...
			if ($is['email'] != ''){
					...
					//Set the subject line
						...
						$text="Ticket $d[no_ticket] has been created, status $statuse and Priority $prio, here's the details : <br>
							<br>
								<table width='50%' cellpadding='0' cellspacing='0' border='0'>                   
								  ...
								  <tr>
								    ...
								    <td align='left' width='2%'>:</td>
								    <td align='left'>$refeedback</td>
								  </tr>
								</table>
 
							 <br>
							<br>
							View Ticket Click a href='http://192.168.31.35/eraticket/content.php?p=wo2&d=$iddata&e=1'>Here</a>
							<br>
							<br>
							<br>
							regards, <br>
